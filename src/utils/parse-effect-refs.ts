...
const STATUS_REF_RE = /\{([^}]+)\}/g;

/** Extract unique status effect reference names from a string. */
export function parseEffectRefs(text: string): string[] {
  const names = new Set<string>();
  let match: RegExpExecArray | null;
  while ((match = STATUS_REF_RE.exec(text)) !== null) {
    names.add(match[1]);
  }
  return [...names];
}

/** Split text into plain text, status-effect reference, skill/talent reference, and italic segments. */
export function splitEffectRefs(text: string): TextSegment[] {
  ...
  while ((match = re.exec(text)) !== null) {
    ...
    if (match[1] != null) {
      ...
      segments.push({ type: 'effectRef', name: match[2] });
