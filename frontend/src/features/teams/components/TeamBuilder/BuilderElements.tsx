...
export function PlaceholderDropItem({
  charKey,
  name,
  charMap,
  note,
...
export function PlaceholderPool({
  ...
  charMap,
  placeholderNotes,
  onPlaceholderNoteChange,
  nameCounts,
}: {
  placeholders: string[];
  charMap: Map<string, Character>;
  placeholderNotes: Record<string, string>;
  onPlaceholderNoteChange: (charKey: string, note: string) => void;
  nameCounts?: Map<string, number>;
}) {
  const { setNodeRef, isOver } = useDroppable({ id: 'placeholder' });
      ...
      <Stack gap="sm">
        ...
        >
          {placeholders.length > 0 ? (
            placeholders.map((name) => {
              ...
              return (
                <PlaceholderDropItem
                  ...
                  name={char?.name ?? name}
                  charMap={charMap}
                  note={placeholderNotes[name] || ''}
                  onNoteChange={onPlaceholderNoteChange}
                  nameCounts={nameCounts}
                />
              );
            })
            ...
            >
              <Text size="xs" c="dimmed">
                Drag characters here to add as budget/placeholder options
              </Text>
            </Box>
          )}
        </SimpleGrid>
